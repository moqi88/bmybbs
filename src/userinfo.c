...
*/
	...
	*/
	...
	{
		getdata(10, 0, "ÇëÑ¡ÔñÄãµÄĞÅÏäÓòÃûĞòºÅ >>  ", tempn, 3, DOECHO, YEA);
		sscanf(tempn, "%d", &n);
	}
	...
	int result;
	getdata(13, 0, "ĞÅÏäÓÃ»§Ãû(ÊäÈëx·ÅÆúÑéÖ¤) >>  ", user, USER_LEN, DOECHO, YEA);
	getdata(14, 0, "ĞÅÏäÃÜÂë >>  ", pass, PASSLEN, DOECHO, YEA);
	...
	while (test_mail_valid(user, pass, IP_POP[n])!=1) {
		...
		getdata(13, 0, "ĞÅÏäÓÃ»§Ãû(ÊäÈëx·ÅÆúÑéÖ¤) >>  ", user, USER_LEN, DOECHO, YEA);
		getdata(14, 0, "ĞÅÏäÃÜÂë >>  ", user, PASSLEN, NOECHO, YEA);
    	}
