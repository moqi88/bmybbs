 ...
 *                
 * $Log: hash_test.c,v $
 * Revision 1.1.1.1  2009-03-04 06:33:28  bmybbs
 * bmysrc
 *
...
int main(int argc, char *argv[])
{
  ...
  if (argc > 1)
    {
      ...
      p_table = ght_create(atoi(argv[1]), ght_one_at_a_time_hash, GHT_HEURISTICS_MOVE_TO_FRONT | GHT_AUTOMATIC_REHASH);
      if (argc > 2)
	i_loops = atoi(argv[2]);

      p_table2 = ght_create(5000, ght_one_at_a_time_hash, GHT_HEURISTICS_MOVE_TO_FRONT | GHT_AUTOMATIC_REHASH);
    }
  else
    {
      ...
      */
      p_table = ght_create(5000, NULL, GHT_HEURISTICS_TRANSPOSE);
      p_table2 = ght_create(5000, NULL, GHT_HEURISTICS_TRANSPOSE);
    }
  ...
  for (i=0; i<i_loops; i++)
    {
      ...
      */
      if ( !(p_entry_data = malloc(sizeof(int))) )
	{
	  perror("malloc");
	  return -errno;
	}
  ...
  for (i=0; i<i_loops; i++)
    {
      ...
      */
      ...
      if (rand()%2 == 1)
	p_he = ght_get(p_table,
		       sizeof(int), &i_key_data);
      else
	p_he = ght_get(p_table2,
		       sizeof(int), &i_key_data);
  ...
  for (i=0; i<i_loops; i++)
    {
      ...
      if (rand()%2 == 1)
	p_he = ght_remove(p_table,
			  sizeof(int), &i_key_data);
      else
	p_he = ght_remove(p_table2,
			  sizeof(int), &i_key_data);
  ...
  for(p_e = ght_first(p_table2, &iterator); p_e; p_e = ght_next(p_table2, &iterator))
    {
      free(p_e);
    }
